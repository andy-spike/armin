...
Service tests against a real temporary Postgres database are the unit test
boundary. Vitest runs the service suites under `src/lib/services/**/*.test.ts`.
...
- create Study Spaces, Decks, Flashcards, prerequisites, archives, deletes, and
...
- promote repeated direct writes into helpers in `src/test/db.ts`
...
first consider moving the durable rule into a service or shared domain module;
otherwise discuss the exception before adding the test.

## E2E Tests

E2E tests provide wiring and journey confidence. Keep them sparse and focused on
flows that prove the hosted app can move real data through the HTTP API and the
renderer.

Use E2E tests to prove critical paths work across HTTP, session, services, and
renderer:
...
Do not mock domain services by default. Prefer real Postgres state and real
service calls.
...
- fixed time
- HTTP sessions or auth seams
- the AI provider (OpenRouter) and any network calls
...
When the data layer, schema, or migrations change:
...
Also run relevant service tests when an AI feature depends on service behavior.
