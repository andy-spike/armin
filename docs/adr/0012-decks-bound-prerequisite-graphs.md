...
Graph reads require a deck id; edge edits stay flashcard-id based and rely on the
shared graph service to enforce same-deck membership. Moving a flashcard to
another deck is allowed, but if it has prerequisite or dependent edges the move
warns that those links will be deleted and affected locks recomputed; the moved
flashcard's saved canvas position resets so the destination deck can place it in
its own layout.
