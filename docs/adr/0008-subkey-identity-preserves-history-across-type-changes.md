...
Related guard: AI-authored cloze content must be normalized to explicit cluster
numbers on write, so positional renumbering of bare `{{…}}` deletions can never
silently churn `c{n}` subKeys and destroy history.
